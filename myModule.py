...
class ImproveModel1(nn.Module):
    def __init__(self, input_dim):
        super(ImproveModel1, self).__init__()
        self.layers = nn.Sequential(
...
class ImproveModel2(nn.Module):
    def __init__(self, num_cities: int, num_numerical_features: int):
        super(ImproveModel2, self).__init__()
        # TODO: 修改模型结构, 注意矩阵的维度（dimensions）
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.embedding_dim + num_numerical_features, 64),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.Dropout(0.2),
            ...
            nn.Linear(16, 1)  # 输出层，预测一个数值
        )

    def forward(self, x):
        city_indices = torch.argmax(x[:, :37], dim=1)
        # 2. 获取 Embedding 向量
        city_vec = self.embedding(city_indices)  # (256, 2)

        # 3. 获取数值特征
        numerical_data = x[:, 37:]  # (256, 79)

        # Embedding 和数值特征拼接到一起
